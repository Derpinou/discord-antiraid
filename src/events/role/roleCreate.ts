...
export const roleCreate: AntiRaidEvent = {
    name: "roleCreate",
    run: async(client: AntiRaid, role: Role) => {
        ...
        try {
            role.guild.fetchAuditLogs({type: client.actionType["roleCreate"].id, limit: 10}).then((audit: GuildAuditLogs) => audit.entries.first()).then(async (entry: GuildAuditLogsEntry | any) => {
